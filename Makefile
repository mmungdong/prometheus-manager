...
PROM-WEB-VERSION:=0.0.1-$(GIT_TAG)$(GIT_MOD)
PROM-SERVER-VERSION:=0.0.1-$(GIT_TAG)$(GIT_MOD)
PROM-AGENT-VERSION:=0.0.1-$(GIT_TAG)$(GIT_MOD)
...
prom-web-docker-push:
	@echo "start push image [${REPO}/prometheus-manager/web:${PROM-WEB-VERSION}]"
	docker push $(PROM-WEB-IMAGE)
	@echo "Successfully push image [${REPO}/prometheus-manager/web:${PROM-WEB-VERSION}] to target repo."

prom-server-docker-build:
	@echo "Building docker image with the prom-server..."
	docker build -t $(PROM-SERVER-IMAGE) -f ./deploy/docker/prom-server/Dockerfile --build-arg VERSION=$(PROM-SERVER-VERSION) .
...
prom-agent-docker-build:
	@echo "Building docker image with the prom-server..."
	docker build -t $(PROM-AGENT-IMAGE) -f ./deploy/docker/prom-agent/Dockerfile --build-arg VERSION=$(PROM-AGENT-VERSION) .
	@echo "Successfully build docker image with the prom-agent."

prom-agent-docker-push:
...
all-docker-compose-up:
	docker-compose -f ./deploy/docker/docker-compose.yaml up $(ARGS)

prom-web-compose-up: clean-server-cache prom-web-docker-build
...
prom-agent-compose-up: clean-server-cache prom-agent-docker-build
